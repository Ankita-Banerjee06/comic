...
export default function Amico() {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const data = await generateAmico(textInput, language);
      setResult(data);
    } catch (err) {
    ...
    <div className="space-y-8 py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-4xl border-2 border-pink-100 shadow-xl p-8 flex flex-col">
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[['💬', t('Speech Bubbles')], ['👤', t('Characters')], ['🎨', t('AI Art')]].map(([e, l]) => (
      ...
      {isProcessing && (
        <div className="bg-white rounded-4xl border-2 border-pink-100 shadow-xl p-12">
          <ProcessingAnimation title={`📚 ${t('Drawing Your Comic')}...`} subtitle={t('Groq AI is writing the script and Pollinations AI is sketching the panels!')} />
        </div>
      )}
          ...
          <div className="bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-4xl p-6 flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-pink-100 font-bold">{t('Your comic strip is ready to read!')}</p>
              </div>
            </div>
