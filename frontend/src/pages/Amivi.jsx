    ...
    <div className="space-y-8 py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-4xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-4xl font-black">{t('AMIVI')} Studio</h1>
              <p className="text-blue-100 font-bold text-lg">{t('SEE IT. Transform text into amazing visuals!')} ✨</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {!isProcessing && !result && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-4xl border-2 border-blue-100 shadow-xl p-8 flex flex-col">
            <h2 className="text-2xl font-black text-gray-800 mb-2">📝 {t('Your Learning Material')}</h2>
            <p className="text-gray-500 font-bold mb-6">{t('Paste any educational text and watch the magic happen!')}</p>
            <textarea
              value={textInput}
              onChange={(e) => setTextInput(e.target.value)}
              placeholder={t('Paste your educational text here... e.g. Photosynthesis is the process by which plants convert sunlight into food...')}
          ...
          <div className="bg-white rounded-4xl border-2 border-purple-100 shadow-xl p-8 flex flex-col">
            <h2 className="text-2xl font-black text-gray-800 mb-2">📄 {t('Or Upload a File')}</h2>
            <p className="text-gray-500 font-bold mb-6">{t('Upload a PDF, Word doc, or image to get started!')}</p>
      ...
      {isProcessing && (
        <div className="bg-white rounded-4xl border-2 border-blue-100 shadow-xl p-12">
          <ProcessingAnimation title={`✨ ${t('Creating Your Visuals')}...`} subtitle={t('Groq AI is generating scripts, Piper is synthesizing voice, and MoviePy is rendering your video!')} />
        </div>
      )}
        ...
        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-4xl p-6 flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-black text-xl">{t('Generation Complete!')}</p>
                <p className="text-green-100 font-bold">{t('Your visuals and video are ready to view!')}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <button
                onClick={async () => {
                  ...
                  try {
                    const quizData = await generateAmiviQuiz(textInput, language);
              ...
              >
                🧩 {isGeneratingQuiz ? t('Generating Quiz') + '...' : t('Take Quiz on this Topic')}
              </button>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            <div className="space-y-4">
              <h3 className="text-2xl font-black text-gray-800 flex items-center gap-2">🎬 {t('Final Video')}</h3>
            ...
            <div className="space-y-4">
              <h3 className="text-2xl font-black text-gray-800 flex items-center gap-2">🖼️ {t('Generated Slides')}</h3>
