...
export default function ComicReader({
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-3xl overflow-hidden flex flex-col h-[480px] sm:h-[600px] lg:h-[700px] shadow-2xl">
      {/* Header Toolbar */}
      <div className="min-h-14 border-b border-gray-800 bg-gray-950/80 backdrop-blur-md flex flex-wrap items-center justify-between gap-y-2 px-3 sm:px-6 py-2 z-10">
        <div className="flex items-center space-x-2 sm:space-x-3">
          <div className="p-1.5 bg-fuchsia-500/20 text-fuchsia-400 rounded-lg">
            <BookOpen className="w-5 h-5" />
          </div>
          <h3 className="font-semibold text-gray-200 text-sm sm:text-base">Generated Comic</h3>
        </div>

        <div className="flex flex-wrap items-center gap-y-2 space-x-1 sm:space-x-3">
          <span className="text-xs sm:text-sm text-gray-400 font-medium bg-gray-800 px-2.5 sm:px-3 py-1 rounded-full whitespace-nowrap">
            Panel {safeIndex + 1} of {panels.length}
          </span>
          <div className="w-px h-6 bg-gray-800 mx-1 sm:mx-2 hidden sm:block"></div>
      ...
      {/* Reader Area */}
      <div className="flex-1 relative bg-[#0f1115] flex items-center justify-center p-3 sm:p-8">
        <button
          onClick={() => setCurrentIndex(p => Math.max(0, p - 1))}
          disabled={safeIndex === 0}
          className="absolute left-1.5 sm:left-6 z-10 p-2 sm:p-3 rounded-full bg-gray-900/80 border border-gray-700 text-white hover:bg-fuchsia-500 hover:border-fuchsia-500 transition-all disabled:opacity-30 disabled:hover:bg-gray-900"
        >
          <ChevronLeft className="w-5 h-5 sm:w-6 sm:h-6" />
        </button>

        {/* The Comic Panel */}
        <div className="w-full max-w-3xl max-h-full aspect-[4/3] bg-white rounded-lg shadow-[0_0_50px_rgba(0,0,0,0.5)] border-4 border-white flex flex-col relative overflow-hidden group">
          <img
            src={imageUrl}
        ...
        <button
          ...
          className="absolute right-1.5 sm:right-6 z-10 p-2 sm:p-3 rounded-full bg-gray-900/80 border border-gray-700 text-white hover:bg-fuchsia-500 hover:border-fuchsia-500 transition-all disabled:opacity-30 disabled:hover:bg-gray-900"
        >
